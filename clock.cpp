...
int main()
{
    ...
    for (wave = 1; wave < 21; wave++)
    {
        ...
        {
            if(find(zombie_type.begin(), zombie_type.end(), 23) != zombie_type.end())
            {
                DealFastWave();
            }
            else
...
void DealSlowAndChangeWave() //慢速和变速关，慢速部分为首4尾2连续冰波
{
    ...
    else if (find(zombies_type.begin(), zombies_type.end(), 32) != zombies_type.end() == false)
    {
        if (find(zombies_type.begin(), zombies_type.end(), 15) != zombies_type.end() &&
            find(zombies_type.begin(), zombies_type.end(), 23) != zombies_type.end())
        {
            DealFastWave();
        }
        else if(find(zombies_type.begin(), zombies_type.end(), 15) != zombies_type.end() &&
            find(zombies_type.begin(), zombies_type.end(), 23) != zombies_type.end()==false)
            {
                DealSFastWave();
            }
        else
